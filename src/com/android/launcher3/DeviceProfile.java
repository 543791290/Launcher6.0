...
public class DeviceProfile {
    ...
    public DeviceProfile(Context context, InvariantDeviceProfile inv, Point minSize, Point maxSize, int width,
            int height, boolean isLandscape) {
    ...
    private void updateIconSize(float scale, int drawablePadding, Resources res, DisplayMetrics dm) {
        ...
        searchBarSpaceHeightPx =
                getSearchBarTopOffset() + res.getDimensionPixelSize(R.dimen.dynamic_grid_search_bar_height);
    ...
    public Rect getSearchBarBounds(boolean isLayoutRtl) {
        ...
        if (isLandscape && transposeLayoutWithOrientation) {
            ...
            if (isTablet) {
                ...
                int gap =
                        (int) ((width - 2 * edgeMarginPx - (inv.numColumns * cellWidthPx)) / (2 * (inv.numColumns + 1)));
    ...
    Rect getWorkspacePadding(boolean isLayoutRtl) {
        ...
        if (isLandscape && transposeLayoutWithOrientation) {
            ...
            if (isTablet) {
                ...
                int availableWidth =
                        Math.max(0, width
                                - (int) ((inv.numColumns * cellWidthPx) + (inv.numColumns * gapScale * cellWidthPx)));
                int availableHeight =
                        Math.max(0, height - paddingTop - paddingBottom - (int) (2 * inv.numRows * cellHeightPx));
    ...
    /**
     * When {@code true}, hotseat is on the bottom row when in landscape mode. If {@code false},
     * hotseat is on the right column when in landscape mode.
     */
