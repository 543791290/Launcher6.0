...
public class DragController {
     ...
     */
    public void startDrag(View v, Bitmap bmp, DragSource source, Object dragInfo, Rect viewImageBounds, int dragAction,
            float initialDragViewScale) {
        int[] loc = mCoordinatesTemp;
        mLauncher.getDragLayer().getLocationInDragLayer(v, loc);
        int dragLayerX =
                loc[0] + viewImageBounds.left + (int) ((initialDragViewScale * bmp.getWidth() - bmp.getWidth()) / 2);
        int dragLayerY =
                loc[1] + viewImageBounds.top + (int) ((initialDragViewScale * bmp.getHeight() - bmp.getHeight()) / 2);
    ...
    public DragView startDrag(Bitmap b, int dragLayerX, int dragLayerY, DragSource source, Object dragInfo,
            int dragAction, Point dragOffset, Rect dragRegion, float initialDragViewScale, boolean accessible) {
                ...
                for (ComponentName componentName : cns) {
                    ...
                    if (dragInfo != null && dragInfo.intent != null) {
                        ...
                        boolean isSameComponent =
                                cn != null && (cn.equals(componentName) || packageNames.contains(cn.getPackageName()));
                        if (isSameComponent) {
                            cancelDrag();
                            return;
                        }
