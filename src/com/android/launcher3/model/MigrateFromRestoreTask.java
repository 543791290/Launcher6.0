...
public class MigrateFromRestoreTask {
    ...
    public void execute() throws Exception {
        ...
        if (!mCarryOver.isEmpty()) {
            ...
            do {
                ...
                OptimalPlacementSolution placement =
                        new OptimalPlacementSolution(new boolean[mTrgX][mTrgY], deepCopy(mCarryOver), true);
                placement.find();
        ...
        final Cursor c =
                mContext.getContentResolver().query(LauncherSettings.Favorites.CONTENT_URI, null, null, null, null);
    ...
    /**
     * Migrate a particular screen id. Strategy: 1) For all possible combinations of row and column,
     * pick the one which causes the least data loss:
     * {@link #tryRemove(int, int, ArrayList, float[])} 2) Maintain a list of all lost items before
     * this screen, and add any new item lost from this screen to that list as well. 3) If all those
     * items from the above list can be placed on this screen, place them (otherwise they are placed
     * on a new screen).
     */
    ...
    private void update(DbEntry item) {
        ...
        mUpdateOperations.add(ContentProviderOperation.newUpdate(LauncherSettings.Favorites.getContentUri(item.id))
        ...
        public void find(int index, float weightLoss, float moveCost, ArrayList<DbEntry> itemsPlaced) {
            ...
            if (me.spanX > 1 || me.spanY > 1) {
                ...
                if (newX < mTrgX && newY < mTrgY) {
                    ...
                    // anyway be same.
                    if (index + 1 < itemsToPlace.size() && itemsToPlace.get(index + 1).weight >= me.weight
                            && !ignoreMove) {
                        find(index + 1, weightLoss + me.weight, moveCost, itemsPlaced);
                    }
                } else {
    ...
    public ArrayList<DbEntry> loadEntries(long screen) {
        Cursor c =
                mContext.getContentResolver().query(
                        ...
                        Favorites.CONTAINER + " = " + Favorites.CONTAINER_DESKTOP + " AND " + Favorites.SCREEN + " = "
                ...
                switch (entry.itemType) {
                    ...
                    case Favorites.ITEM_TYPE_APPWIDGET: {
                        ...
                        LauncherAppWidgetProviderInfo pInfo =
                                LauncherModel.getProviderInfo(mContext, cn, UserHandleCompat.myUserHandle());
    ...
    private int getFolderItemsCount(long folderId) {
        Cursor c =
                mContext.getContentResolver().query(LauncherSettings.Favorites.CONTENT_URI,
                        new String[] {Favorites._ID, Favorites.INTENT}, Favorites.CONTAINER + " = " + folderId, null,
                        null, null);
    ...
    private static class DbEntry extends ItemInfo implements Comparable<DbEntry> {
        ...
        /**
         * Comparator such that larger widgets come first, followed by all 1x1 items based on their
         * weights.
         */
        @Override
        ...
        public boolean columnsSame(DbEntry org) {
            return org.cellX == cellX && org.cellY == cellY && org.spanX == spanX && org.spanY == spanY
                    && org.screenId == screenId;
        }
    ...
    public static void markForMigration(Context context, int srcX, int srcY, HashSet<String> widgets) {
        prefs(context).edit().putString(KEY_MIGRATION_SOURCE_SIZE, srcX + "," + srcY)
                .putStringSet(KEY_MIGRATION_WIDGET_MINSIZE, widgets).apply();
    }
    ...
    public static void clearFlags(Context context) {
        prefs(context).edit().remove(KEY_MIGRATION_SOURCE_SIZE).remove(KEY_MIGRATION_WIDGET_MINSIZE).commit();
    }

    private static SharedPreferences prefs(Context context) {
        return context.getSharedPreferences(LauncherAppState.getSharedPreferencesKey(), Context.MODE_PRIVATE);
    }
}
