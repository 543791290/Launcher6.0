 ...
 * <li>sparse matrix of cells that are 1x1 [ 1][ ][ 2][ ] [ ][ ][ 3][ ] [ ][ 4][ ][ ] [ ][ 5][ 6][
 * 7]
 * </ul>
 * *
 * <p>
 * For testing, one can use a BT keyboard, or use following adb command. ex. $ adb shell input
 * keyevent 20 // KEYCODE_DPAD_LEFT
 */
public class FocusLogic {
    ...
    public static int handleKeyEvent(int keyCode, int cntX, int cntY, int[][] map, int iconIdx, int pageIndex,
            int pageCount, boolean isRtl) {

        if (DEBUG) {
            Log.v(TAG, String.format("handleKeyEvent START: cntX=%d, cntY=%d, iconIdx=%d, pageIdx=%d, pageCnt=%d",
    ...
    /**
     * Returns a matrix of size same as the {@link CellLayout} dimension that is initialized with
     ...
     * The size of the returning matrix is [icon column count x (icon + hotseat row count)] in
     * portrait orientation. In landscape, [(icon + hotseat) column count x (icon row count)]
     */
    // TODO: get rid of the dynamic matrix creation
    public static int[][] createSparseMatrix(CellLayout iconLayout, CellLayout hotseatLayout, boolean isHorizontal,
            int allappsiconRank, boolean includeAllappsicon) {
    ...
    /**
     * Creates a sparse matrix that merges the icon of previous/next page and last column of current
     * page. When left key is triggered on the leftmost column, sparse matrix is created that
     * combines previous page matrix and an extra column on the right. Likewise, when right key is
     * triggered on the rightmost column, sparse matrix is created that combines this column on the
     * 0th column and the next page matrix.
     *
     ...
     *
     * Example of the check order for KEYCODE_DPAD_RIGHT: [ ][ ][13][14][15] [ ][ 6][ 8][10][12] [
     * X][ 1][ 2][ 3][ 4] [ ][ 5][ 7][ 9][11]
     */
     ...
     *
     * Example of the check order for KEYCODE_DPAD_DOWN: [ ][ ][ ][ X][ ][ ][ ] [ ][ ][ 5][ 1][ 4][
     * ][ ] [ ][10][ 7][ 2][ 6][ 9][ ] [14][12][ 9][ 3][ 8][11][13]
     */
    // TODO: add unit tests to verify all permutation.
     ...
     * @return the view adjacent to
     * @param oldView in the
     * @param nextPage .
     */
    public static View getAdjacentChildInNextPage(ShortcutAndWidgetContainer nextPage, View oldView, int edgeColumn) {
        ...
        int column =
                (edgeColumn == NEXT_PAGE_LEFT_COLUMN) ^ nextPage.invertLayoutHorizontally()
                        ? 0
