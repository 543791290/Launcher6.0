...
public class FocusHelper {
    ...
    public static class PagedFolderKeyEventListener implements View.OnKeyListener {
        ...
        public boolean onKey(View v, int keyCode, KeyEvent e) {
            ...
            // Process focus.
            int newIconIndex =
                    FocusLogic.handleKeyEvent(keyCode, countX, countY, matrix, iconIndex, pageIndex, pageCount,
                            isLayoutRtl);
                    ...
                    if (newParent != null) {
                        ...
                        child =
                                newParent.getChildAt(
                                        ((newIconIndex == FocusLogic.PREVIOUS_PAGE_LEFT_COLUMN) ^ newParent
                                                .invertLayoutHorizontally()) ? 0 : countX - 1, row);
                    }
                    break;
     ...
     * <p>
     * Currently we don't special case for the phone UI in different orientations, even though the
    ...
    static boolean handleHotseatButtonKeyEvent(View v, int keyCode, KeyEvent e) {
        ...
        if (DEBUG) {
            Log.v(TAG,
                    String.format("Handle HOTSEAT BUTTONS keyevent=[%s] on hotseat buttons, isVertical=%s",
        ...
        int iconRank =
                ((CellLayout.LayoutParams) hotseatLayout.getShortcutsAndWidgets().getChildAt(iconIndex)
                        .getLayoutParams()).cellX;
        ...
        if (keyCode == KeyEvent.KEYCODE_DPAD_UP && !profile.isVerticalBarLayout()) {
            matrix =
                    FocusLogic.createSparseMatrix(iconLayout, hotseatLayout, true /*
                                                                                   * hotseat
                                                                                   * horizontal
            ...
            matrix =
                    FocusLogic.createSparseMatrix(iconLayout, hotseatLayout, false /*
                                                                                    * hotseat
                                                                                    * horizontal
                                                                                    */, profile.inv.hotseatAllAppsRank,
                            iconRank == profile.inv.hotseatAllAppsRank /* include all apps icon */);
    ...
    static boolean handleIconKeyEvent(View v, int keyCode, KeyEvent e) {
        ...
        if (DEBUG) {
            Log.v(TAG,
                    String.format("Handle WORKSPACE ICONS keyevent=[%s] isVerticalBar=%s",
                            KeyEvent.keyCodeToString(keyCode), profile.isVerticalBarLayout()));
        }
        ...
        if (keyCode == KeyEvent.KEYCODE_DPAD_DOWN && !profile.isVerticalBarLayout()) {
            matrix =
                    FocusLogic.createSparseMatrix(iconLayout, hotseatLayout, true /* horizontal */,
                            profile.inv.hotseatAllAppsRank, !hotseat.hasIcons() /*
                                                                                 * ignore all apps
                                                                                 * icon, unless
                                                                                 * there are no
                                                                                 * other icons
            ...
            matrix =
                    FocusLogic.createSparseMatrix(iconLayout, hotseatLayout, false /* horizontal */,
                            profile.inv.hotseatAllAppsRank, !hotseat.hasIcons() /*
                                                                                 * ignore all apps
                                                                                 * icon, unless
                                                                                 * there are no
        ...
        switch (newIconIndex) {
            ...
            case FocusLogic.NEXT_PAGE_RIGHT_COLUMN:
                ...
                if (parent != null) {
                    ...
                    newIconIndex =
                            FocusLogic.handleKeyEvent(keyCode, countX + 1, countY, matrix, FocusLogic.PIVOT,
