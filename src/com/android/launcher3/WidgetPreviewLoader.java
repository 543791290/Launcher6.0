...
public class WidgetPreviewLoader {
     ...
     * finalized, and then reclaimed. Note: synchronized block used for this variable is expensive
     * and the block should always be posted to a background thread.
     */
    @Thunk
     ...
     */
    public PreviewLoadRequest getPreview(final Object o, int previewWidth, int previewHeight, WidgetCell caller) {
    ...
    private static class CacheDb extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase database) {
            database.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " (" + COLUMN_COMPONENT + " TEXT NOT NULL, "
                    + COLUMN_USER + " INTEGER NOT NULL, " + COLUMN_SIZE + " TEXT NOT NULL, " + COLUMN_PACKAGE
                    + " TEXT NOT NULL, " + COLUMN_LAST_UPDATED + " INTEGER NOT NULL DEFAULT 0, " + COLUMN_VERSION
                    + " INTEGER NOT NULL DEFAULT 0, " + COLUMN_PREVIEW_BITMAP + " BLOB, " + "PRIMARY KEY ("
                    + COLUMN_COMPONENT + ", " + COLUMN_USER + ", " + COLUMN_SIZE + ") " + ");");
        }
    ...
    private WidgetCacheKey getObjectKey(Object o, String size) {
        ...
        if (o instanceof LauncherAppWidgetProviderInfo) {
            ...
            return new WidgetCacheKey(new ComponentName(info.activityInfo.packageName, info.activityInfo.name),
    ...
    public void removeObsoletePreviews(ArrayList<Object> list) {
        ...
        try {
            c =
                    mDb.getReadableDatabase().query(
                            CacheDb.TABLE_NAME,
                            new String[] {CacheDb.COLUMN_USER, CacheDb.COLUMN_PACKAGE, CacheDb.COLUMN_LAST_UPDATED,
                                    CacheDb.COLUMN_VERSION}, null, null, null, null, null);
            while (c.moveToNext()) {
            ...
            cursor =
                    mDb.getReadableDatabase().query(
                            ...
                            CacheDb.COLUMN_COMPONENT + " = ? AND " + CacheDb.COLUMN_USER + " = ? AND "
                                    + CacheDb.COLUMN_SIZE + " = ?",
                            new String[] {key.componentName.flattenToString(),
    ...
    public Bitmap generateWidgetPreview(Launcher launcher, LauncherAppWidgetProviderInfo info, int maxPreviewWidth,
        ...
        if (widgetPreviewExists) {
            ...
            try {
                ...
                if (icon != null) {
                    ...
                    icon.setBounds(hoffset, yoffset, hoffset + (int) (appIconSize * iconScale), yoffset
    ...
    private Bitmap generateShortcutPreview(Launcher launcher, ResolveInfo info, int maxWidth, int maxHeight,
            Bitmap preview) {
        final Canvas c = new Canvas();
        if (preview == null) {
