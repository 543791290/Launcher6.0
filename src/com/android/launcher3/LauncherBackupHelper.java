...
public class LauncherBackupHelper implements BackupHelper {
    ...
    /**
     * @return true if the backup corresponding to oldstate can be successfully applied to this
     *         device.
     */
    private boolean isBackupCompatible(Journal oldState) {
        ...
        if ((currentProfile.allappsRank >= oldProfile.allappsRank)
                && ((currentProfile.hotseatCount - currentProfile.allappsRank) >= (oldProfile.hotseatCount - oldProfile.allappsRank))) {
        ...
        if (MigrateFromRestoreTask.ENABLED && (oldProfile.desktopCols - currentProfile.desktopCols <= 1)
    ...
    /**
     * Restore launcher configuration from the restored data stream. It assumes that the keys will
     * arrive in lexical order. So if the journal was present in the backup, it should arrive first.
     *
    ...
    private void restoreIcon(Key key, byte[] buffer, int dataSize) throws IOException {
        ...
        if (icon == null) {
            ...
            mIconCache.preloadIcon(ComponentName.unflattenFromString(key.name), icon, res.dpi, "" /* label */,
                    mUserSerial, mIdp);
        }
    }
    ...
    private void restoreWidget(Key key, byte[] buffer, int dataSize) throws IOException {
        ...
        if (widget.icon.data != null) {
            ...
            if (icon == null) {
                ...
                mIconCache.preloadIcon(ComponentName.unflattenFromString(widget.provider), icon, widget.icon.dpi,
                        widget.label, mUserSerial, mIdp);
            }
        }
    ...
    /**
     * @return true if its an hotseat item, that can be replaced during restore. TODO: Extend check
     *         for folders in hotseat.
     */
    private boolean isReplaceableHotseatItem(Favorite favorite) {
        return favorite.container == Favorites.CONTAINER_HOTSEAT
                ...
                && (favorite.itemType == Favorites.ITEM_TYPE_APPLICATION || favorite.itemType == Favorites.ITEM_TYPE_SHORTCUT);
    }
    ...
    private ContentValues unpackFavorite(byte[] buffer, int dataSize) throws IOException {
        ...
        // grid migration.
        DeviceProfieData currentProfile =
                migrationCompatibleProfileData == null ? mDeviceProfileData : migrationCompatibleProfileData;
        ...
        if (favorite.itemType == Favorites.ITEM_TYPE_APPWIDGET) {
            ...
            if (isReplaceableHotseatItem(favorite) && favorite.targetType != Favorite.TARGET_NONE
    ...
    /**
     * A class to check if an activity can handle one of the intents from a list of predefined
     * intents.
     */
    private class ItemTypeMatcher {
        ...
        public boolean matches(ActivityInfo activity, PackageManager pm) {
            for (Intent intent : mIntents) {
                ...
                if (info != null
                        && (info.activityInfo.name.equals(activity.name) || info.activityInfo.name
                                .equals(activity.targetActivity))) {
                    return true;
                }
            }
            return false;
