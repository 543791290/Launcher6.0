...
public class SearchDropTargetBar extends FrameLayout implements DragController.DragListener {
    ...
    public enum State {
        INVISIBLE(0f, 0f), SEARCH_BAR(1f, 0f), DROP_TARGET(0f, 1f);
    ...
    /**
     * Animates the current search bar state to a new state. If the
     * 
     * @param duration is 0, then the state is applied immediately.
     */
     ...
     */
    private void animateViewAlpha(LauncherViewPropertyAnimator animator, View v, float alpha, int duration) {
        if (v == null) {
            return;
        }
