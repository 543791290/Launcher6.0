...
public class FolderPagedView extends PagedView {
    ...
    /**
     * Sets up the grid size such that
     * 
     * @param count items can fit in the grid. The grid size is calculated such that countY <=
     *        countX and countX = ceil(sqrt(count)) while maintaining the restrictions of
     *        {@link #mMaxCountX} &amp; {@link #mMaxCountY}.
     */
     ...
     * Adds the
     * 
     * @param view to the layout based on
     * @param rank and updated the position related attributes. It assumes that
     * @param item is already attached to the view.
     */
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            if (v != null) {
                ...
                if (info.cellX != newX || info.cellY != newY || info.rank != rank) {
                    ...
                    info.rank = rank;
                    if (saveChanges) {
                        LauncherModel.addOrMoveItemInDatabase(getContext(), info, mFolder.mInfo.id, 0, info.cellX,
                                info.cellY);
                    }
                }
                lp.cellX = info.cellX;
    ...
    public int findNearestArea(int pixelX, int pixelY) {
        ...
        return Math.min(mAllocatedContentSize - 1, pageIndex * mMaxItemsPerPage + sTempPosArray[1] * mGridCountX
                + sTempPosArray[0]);
    }
    ...
    public String getAccessibilityDescription() {
        return String.format(getContext().getString(R.string.folder_opened), mGridCountX, mGridCountY);
    }
    ...
    public void showScrollHint(int direction) {
        float fraction =
                (direction == DragController.SCROLL_LEFT) ^ mIsRtl ? -SCROLL_HINT_FRACTION : SCROLL_HINT_FRACTION;
    ...
    /**
     * Reorders the items such that the
     * 
     * @param empty spot moves to
     * @param target
     */
    public void realTimeReorder(int empty, int target) {
        ...
        while (moveStart != moveEnd) {
            ...
            if (v != null) {
                if (pageToAnimate != p) {
                    ...
                    v.animate().translationXBy((direction > 0 ^ mIsRtl) ? -v.getWidth() : v.getWidth())
                            .setDuration(REORDER_ANIMATION_DURATION).setStartDelay(0).withEndAction(endAction);
        ...
        for (int i = startPos; i != endPos; i += direction) {
            ...
            if (page.animateChildToPosition(v, i % mGridCountX, i / mGridCountX, REORDER_ANIMATION_DURATION, delay,
                    true, true)) {
                delay += delayAmount;
    ...
    public void animateMarkers() {
        ...
        for (int i = 0; i < count; i++) {
            mPageIndicator.getChildAt(i).animate().scaleX(1).scaleY(1).setInterpolator(interpolator)
                    .setDuration(PAGE_INDICATOR_ANIMATION_DURATION)
                    .setStartDelay(PAGE_INDICATOR_ANIMATION_STAGGERED_DELAY * i + PAGE_INDICATOR_ANIMATION_START_DELAY);
        }
    }
