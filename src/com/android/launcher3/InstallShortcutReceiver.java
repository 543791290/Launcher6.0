...
public class InstallShortcutReceiver extends BroadcastReceiver {
    ...
    public static void removeFromInstallQueue(Context context, ArrayList<String> packageNames, UserHandleCompat user) {
    ...
    private static class PendingInstallShortcutInfo {
        ...
        public String encodeToString() {
            if (activityInfo != null) {
                try {
                    ...
                    // recreate this.
                    return new JSONStringer().object().key(LAUNCH_INTENT_KEY).value(launchIntent.toUri(0))
                            .key(APP_SHORTCUT_TYPE_KEY).value(true).key(USER_HANDLE_KEY)
                            .value(UserManagerCompat.getInstance(mContext).getSerialNumberForUser(user)).endObject()
                            .toString();
            ...
            try {
                JSONStringer json =
                        new JSONStringer().object().key(LAUNCH_INTENT_KEY).value(launchIntent.toUri(0)).key(NAME_KEY)
                                .value(name);
                if (icon != null) {
        ...
        public String getTargetPackage() {
            ...
            if (packageName == null) {
                packageName = launchIntent.getComponent() == null ? null : launchIntent.getComponent().getPackageName();
            }
            return packageName;
        }
        ...
        try {
            ...
            if (object.optBoolean(APP_SHORTCUT_TYPE_KEY)) {
                ...
                UserHandleCompat user =
                        UserManagerCompat.getInstance(context).getUserForSerialNumber(object.getLong(USER_HANDLE_KEY));
                if (user == null) {
                    return null;
                }
